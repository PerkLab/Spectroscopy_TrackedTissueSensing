...
%
%   Inputs:
%     filename: metaimage filename (.mha or .mhd)
%     requestedTransforms: optional cell array of transform names, if specified then
%       only these transforms will be read, the others will be ignored;
%       if unspecified then all the transforms are read
%
%   Outputs:
...
function [transforms timestamps unfilteredTimestamps] = mha_read_transforms(filename, requestedTransforms)
...
while 1
  ...
  if (length(token)>0)
	  ...
	  if ( ~isempty(requestedTransforms) & isempty(find(strcmp(requestedTransforms, transformName))) )
	    % specific transforms have been requested, but transformName is not one of them
	    continue;
	  end
